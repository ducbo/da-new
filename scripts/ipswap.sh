...
swapOLSfile(){
	...
	OLD_IP_DASHED=`echo ${OLD_IP} | tr '.' '-'`
	NEW_IP_DASHED=`echo ${NEW_IP} | tr '.' '-'`
	perl -pi -e 's|${OLD_IP_DASHED}-|${NEW_IP_DASHED}-|g' $1
	log "$1\t: $OLD_IP -> $NEW_IP"
}
...
if [ "${HAVE_OPENLITESPEED}" -eq 1 ]; then
	...
	swapOLSfile /usr/local/lsws/conf/ips.conf
	swapOLSfile /usr/local/lsws/conf/listeners.conf
	swapOLSfile /usr/local/lsws/conf/httpd-vhosts.conf
fi
